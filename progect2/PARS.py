...
def D():
    ...
    for i in rr_intervals:
        sumNN += (i - mean) ** 2
    return sumNN/(len(rr_intervals) - 1)
...
def PARS():
    ...
    return (N1(mean()*1000), N2(SDNN()*1000, variation_scale()/mean(), CV()),
           N3(variation_scale()*1000, mode2()[0], tension_index()), N4(CV()),
            N5(*norm(fft_res)))
...
print(PARS())
count = 0
count = [count + abs(x) for x in PARS()]
print(sum(count))
